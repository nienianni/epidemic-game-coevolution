...
def theory(args):
    num_agebrackets, ages, seed_num, initial_infected_age, lambda1, lambda2, r, M, H, t_max, game_t_max, x0_ABCD_list, G_A_percentage, G_B_percentage, G_C_percentage, G_D_percentage_list, beta = args
    R_GD_list = []
    R_A_GD_list = []
    R_B_GD_list = []
    R_C_GD_list = []
    ...
    for G_D in tqdm(G_D_percentage_list):
        game_t = 1
        i = 0
        label = 0
        # 初始化--设置初始接种
        ...
        while game_t < game_t_max:
            ...
            ### 判断博弈是否停止
            if game_t > 1:
                if abs(Rv_A_t_list[i] - Rv_A_t_list[i - 1]) < 0.00000001 and abs(Rv_B_t_list[i] - Rv_B_t_list[i - 1]) < 0.000001 and abs(Rv_C_t_list[i] - Rv_C_t_list[i - 1]) < 0.000001 and abs(Rv_D_t_list[i] - Rv_D_t_list[i - 1]) < 0.000001 and label == 0:
                    game_t = game_t_max - 2
                    label = 1

            ### 博弈+疫苗接种
            game_theory_model.get_dx(beta,G_A_percentage,G_B_percentage,G_C_percentage,G_D)
            game_t += 1
            i+=1
